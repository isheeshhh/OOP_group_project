...
# Store project paths used for assets and the current user session.
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
ASSET_DIR = os.path.join(SCRIPT_DIR, "assets")
SESSION_FILE = os.path.join(SCRIPT_DIR, "current_user.json")
USERS_FILE = os.path.join(SCRIPT_DIR, "users.json")
COURSE_DATABASE_FILE = os.path.join(SCRIPT_DIR, "courses_students.json")

def asset(name):
    """Helper function to load assets from the assets folder."""
    return os.path.join(ASSET_DIR, name)

# Loads the professor account that logged in from homepage.py.
def load_current_user():
    try:
        with open(SESSION_FILE, "r", encoding="utf-8") as file:
            return json.load(file)
    except (FileNotFoundError, json.JSONDecodeError):
        return {"fullname": "Professor", "username": "professor", "role": "professor"}

# Get the logged-in professor name for the dashboard header.
current_user = load_current_user()
prof_name = "Prof. " + current_user.get("fullname", "Professor")
professor_id = current_user.get("email", current_user.get("username", "professor")).strip().lower()


def empty_course_database():
    """Returns the default course database structure."""
    return {"professors": {}}


def load_course_database():
    """Loads courses and enrolled students from the JSON database."""
    if not os.path.exists(COURSE_DATABASE_FILE):
        save_course_database(empty_course_database())

    try:
        with open(COURSE_DATABASE_FILE, "r", encoding="utf-8-sig") as file:
            data = json.load(file)
    except (json.JSONDecodeError, OSError):
        data = empty_course_database()

    if "professors" not in data or not isinstance(data["professors"], dict):
        data = empty_course_database()

    data["professors"].setdefault(professor_id, {"courses": []})
    return data


def save_course_database(data):
    """Saves the course database to disk."""
    with open(COURSE_DATABASE_FILE, "w", encoding="utf-8") as file:
        json.dump(data, file, indent=4)


def load_courses():
    """Returns the logged-in professor's saved courses."""
    data = load_course_database()
    return data["professors"][professor_id].setdefault("courses", [])


def save_courses():
    """Persists the logged-in professor's courses."""
    data = load_course_database()
    data["professors"][professor_id] = {
        "professor_name": current_user.get("fullname", "Professor"),
        "courses": courses
    }
    save_course_database(data)


def load_student_accounts():
    """Loads student accounts that can be enrolled in a course."""
    try:
    ...
    for user in data.get("users", []):
        if user.get("role", "").strip().lower() == "student":
            students.append({
                "fullname": user.get("fullname", "Student"),
                "email": user.get("email", user.get("username", "")).strip().lower()
            })
    return students


def student_label(student):
    """Formats a student for display in the professor UI."""
    email = student.get("email", "")
    return f"{student.get('fullname', 'Student')} ({email})" if email else student.get("fullname", "Student")
...
def open_manage_courses(edit_idx=None):
    ...
    if student_accounts:
        for student in student_accounts:
            student_listbox.insert(END, student_label(student))
    else:
    ...
    if is_edit:
        ...
        selected_emails = {student.get("email", "") for student in c_data.get("students", [])}
        for index, student in enumerate(student_accounts):
            if student.get("email", "") in selected_emails:
                student_listbox.selection_set(index)
    ...
    def save_course():
        ...
        selected_students = []
        if student_accounts:
            for index in student_listbox.curselection():
                selected_students.append(student_accounts[index])
        ...
        if is_edit:
            ...
            courses[edit_idx]["students"] = selected_students
